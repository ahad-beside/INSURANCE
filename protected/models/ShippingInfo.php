...
class ShippingInfo extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('user_id_fk, name, street_address, landmark, city, pincode, phone, update_by', 'required'),
            array('user_id_fk, city, country, update_by', 'numerical', 'integerOnly' => true),
            ...
            array('id, user_id_fk, name, street_address, landmark, city, country, pincode, phone, update_time, update_by', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function relations() {
        ...
        return array(
            ...
            'cityIdFk' => array(self::BELONGS_TO, 'City', 'city'),
        );
    }
    ...
    public static function getShippingInfo($userId){
        ...
        if(count($info)>0){
            return CHtml::encode($info->name.', '.$info->street_address.', '.$info->landmark.', '.$info->cityIdFk->name.', '.$info->pincode.', '.$info->phone.', '.Yii::app()->params->countryName);
        }else{
            return '';
        }
    }
